...
BOOT_WAIT=3    # seconds to wait after app launch
...
capture_logs() {
    ...
    xcrun simctl spawn "$SIM_UDID" log show \
        ...
        --style compact \
        --last 2m \
        2>/dev/null > "$output_file" 2>&1 || true

    # Secondary: also capture CrashReporter entries specifically about our app
    local crash_file="$LOGS_DIR/${card_name}-crashes.txt"
    xcrun simctl spawn "$SIM_UDID" log show \
        --start "$start_time" \
...
diagnose_logs() {
    ...
    local app_lines
    app_lines=$(grep -ciE "AdaptiveCard" "$logfile" 2>/dev/null || echo "0")
    ...
    if grep -qiE "AdaptiveCard.*(watchdog|hang|timeout|not respond)" "$logfile" 2>/dev/null; then
...
get_memory_info() {
    # Try to get memory via simctl spawn and footprint utility
    local mem_bytes
    mem_bytes=$(xcrun simctl spawn "$SIM_UDID" footprint -a 2>/dev/null | grep -i "$APP_ID" | awk '{print $NF}' | head -1 || true)
    ...
    # Fallback: use simctl diagnostics (memory_pressure / vm_stat)
    # or parse from log stream for memory warnings
    local mem_warn
    mem_warn=$(xcrun simctl spawn "$SIM_UDID" log show --last 30s \
        --predicate "category == 'memory' OR messageType == 'fault'" \
        --style compact 2>/dev/null | grep -ci "memory" || true)

    if [ "$mem_warn" -gt 5 ] 2>/dev/null; then
        echo "HIGH_PRESSURE"
    else
        echo "0"
    fi
...
phase4_crash_analysis() {
    ...
    for logfile in "$LOGS_DIR"/*.txt; do
        ...
        if grep -qiE "watchdog|hang|timeout" "$logfile" 2>/dev/null; then
